...
public class Aeps1ReportServiceImpl implements Aeps1ReportService {
    ...
    public PaginatedResponse<Aeps1ReportResponseDTO> fetchAepsReport(Aeps1ReportRequestDTO requestDTO) {
        ...
        Page<Aeps1ReportEntity> paginatedContent = aeps1ReportRepository.fetchAepsReport(requestDTO.getStartDate(), requestDTO.getEndDate(), requestDTO.getOrderId(), requestDTO.getTxnId(), requestDTO.getTxnType(), requestDTO.getStatus(), requestDTO.getSearch(), requestDTO.getSearchByField(), page);

        List<Aeps1ReportResponseDTO> responseDTOS = new ArrayList<>();
        paginatedContent.getContent().forEach(content -> responseDTOS.add(aeps1ReportMapper.toResponseDto(content)));
